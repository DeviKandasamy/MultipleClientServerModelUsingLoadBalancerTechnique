...
public class Server1 {
	...
	Server1(int serverListenPort, int lbPort, int statusPort )  {
		String s;
		...
		StatusThread1 sThreadObj1 = new StatusThread1(statusPort);
		Thread statusThread = new Thread(sThreadObj1);
		statusThread.start();
		...
		System.out.println(getCurrentTime() + "abcd::" +"Waiting for client connections");
		while (true) {
			try {
			...
			ServerConnectionThread1 serverconnThreadObj1 = new ServerConnectionThread1 (serverAcceptSocket );
			// Pick a server socket to use with this client connection
			//Create a Thread, call Thread start
			Thread  connThread = new Thread(serverconnThreadObj1);
			connThread.start();
		}
	}
...
class ServerConnectionThread1 implements Runnable{
	ResultSet res;
	Connection conn;
	Socket clientSocket;
	DataInputStream clientIn;
	DataOutputStream clientOut;
	ServerConnectionThread1(Socket s1) {
		clientSocket = s1;
	}
	public void run() {
		String s;
	...
	public void GetDetails(String query)
	{
		try
		{
			...
			System.out.println(Server1.getCurrentTime() + "1::" + "Sql query : "+NewCommand);
			...
			System.out.println(Server1.getCurrentTime() + "1::" + "Patient Details from DATABASE");
...
class StatusThread1 implements Runnable{
	...
	StatusThread1(int portforStatus) {
	...
	public void run() {
		...
		while (true) {
			try {
				...
				System.out.println(Server1.getCurrentTime() + "1::" + "Incoming connection from Loadbalancer");
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			...
			try {
				...
				s = lbIn.readUTF();
				System.out.println(Server1.getCurrentTime() + "1::" +"Load balancer says " + s);
				lbOut.close();
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
